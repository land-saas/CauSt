...
| GraphST / SpaGCN backbones | ✅ GraphST (plain-torch port, `backend: graphst`); SpaGCN planned | `caust.models.graphst` |
...
| Strategies at equal K: HVG-K, High-δ-K (λ=0), CauST-K — plus donor-aware HVG, Moran's I, random controls | ✅ | `caust.transfer.gene_set` |
...
| Within-slice / within-donor / cross-donor ARI and NMI, generalization gap, win counts, paired Wilcoxon tests | ✅ | `caust.transfer.summarize` |
...
| Mouse brain (Visium, 2 sections) | ⏳ planned — manual labels exist for Sagittal-Anterior Section 1 only |
| MERFISH hypothalamus | ✅ 5 Bregma sections of one animal, 8 BASS-annotated regions (`data.source: merfish`) |
| STARmap visual cortex | ✅ 1 section, 7 domains (`starmap_vc`); plus STARmap PFC, 3 sections, 4 layers (`starmap_pfc`) |
...
| Multi-backbone integration (GraphST, SpaGCN) | 🟡 GraphST done; SpaGCN planned |
...
| Scanpy-style API (`caust.tl` / `caust.pl`), tutorial notebook (DLPFC, executed) | ✅ MkDocs site; ReadTheDocs hosting pending |
